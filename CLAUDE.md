...
- [ ] **Progressive overload display (deferred from V2)** — "Last time: 42.5kg × 10" in set editor. Design decision pending: (A) per-Day saved-set history via a LoggedSet[] log, or (B) cross-Day visibility using last-saved values. (A) aligns with Weekly Plans §3.1 `WeekPlan.exerciseLogs`; revisit when Weekly Plans lands.
